...
def mostRecent():
	filt = []	
	rcURL = "https://en.wikipedia.org/w/api.php?action=query&list=recentchanges&rcprop=title|ids|sizes|flags|user|comment&rclimit=500&format=xml"
	response = urllib2.urlopen(rcURL).read()
	tree = ET.parse(response)
	root = tree.getroot()	
	#print(root.text)
	for child in root.findall('edit'):
		filt += child
